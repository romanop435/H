...
class WikiParser:
    """
    Использует официальный JSON-список всех страниц wiki:
      https://wiki.facepunch.com/gmod/~pagelist?format=json
    Это даёт точный список ~6300 страниц без мусора из навигации.
    """

    # Префиксы страниц которые нас интересуют (GLua API)
    INTERESTING_PREFIXES = [
        "hook.", "player.", "Player:", "Entity:", "game.", "util.",
        "net.", "timer.", "table.", "string.", "math.", "surface.",
        "draw.", "render.", "file.", "http.", "vgui.", "input.",
        "weapons.", "team.", "ents.", "constraint.", "effects.",
        "sound.", "resource.", "numpad.", "system.", "os.",
        "GM:", "GAMEMODE:", "SWEP.", "ENT.", "SANDBOX:",
        "DFrame", "DPanel", "DButton", "DLabel", "DTextEntry",
        "DScrollPanel", "DListView", "DComboBox", "DNumSlider",
        "DCheckBox", "DMenu", "DImage", "DModelPanel",
        "Angle:", "Vector:", "Color", "PhysObj:",
        "NPC:", "Vehicle:", "Weapon:",
        "hook_", "Using_Hooks", "Networking", "HUD_", "SWEP_Creation",
        "Entity_Creation", "Gamemode_Creation", "Beginner_Tutorial",
        "DarkRP", "TTT_", "Sandbox_",
    ]

    # Страницы которые точно пропускаем
    SKIP_KEYWORDS = [
        "~", "Help/", "Special:", "Category:", "changelog",
        "Achievements_List", "FAQ", "Main_Menu", "Finding_",
        "Using_your_", "Addons_Menu", "Addons_Usage",
    ]
    ...
    def get_all_page_slugs(self):
        """Получает JSON-список всех страниц wiki."""
        url = f"{WIKI_BASE}/gmod/~pagelist?format=json"
        try:
            r = self.session.get(url, timeout=15)
            if r.status_code == 200:
                data = r.json()
                # Формат: список строк (slug) или список объектов
                if isinstance(data, list):
                    ...
                    for item in data:
                        if isinstance(item, str):
                            ...
                            slugs.append(item.get("title") or item.get("slug") or "")
                    return [s for s in slugs if s]
        except Exception as e:
            print(f"  ⚠️  Ошибка получения списка страниц: {e}")
        return []

    def is_interesting(self, slug: str) -> bool:
        """Проверяет стоит ли парсить эту страницу."""
        # Пропускаем служебные страницы
        for skip in self.SKIP_KEYWORDS:
            if skip.lower() in slug.lower():
                return False
        # Берём страницы с GLua API
        for prefix in self.INTERESTING_PREFIXES:
            if slug.startswith(prefix) or prefix in slug:
                return True
        # Также берём страницы с точкой или двоеточием (функции/методы)
        if "." in slug or ":" in slug:
            return True
        return False

    def parse_page(self, slug: str) -> dict | None:
        """Парсит одну страницу wiki по slug."""
        url = f"{WIKI_BASE}/gmod/{slug}"
        ...
        for el in soup.find_all(["pre", "code"]):
            c = el.get_text().strip()
            # Только GLua-подобный код (не просто числа или HTML)
            if len(c) > 20 and any(kw in c for kw in [
                "function", "hook", "end", "local", "if ", "for ",
                "return", "Player", "Entity", "print("
            ]):
        ...
        return_info = ""
        for el in soup.find_all(["div", "p"], string=re.compile(r"[Rr]eturn")):
            return_info = clean_text(el.get_text())[:200]
            break
    ...
    def parse_all(self, max_pages=800):
        print("\n📖 Парсим GLua Wiki через pagelist API...")

        # Шаг 1: получаем полный список страниц
        all_slugs = self.get_all_page_slugs()
        print(f"  📋 Всего страниц в wiki: {len(all_slugs)}")

        # Шаг 2: фильтруем интересные
        interesting = [s for s in all_slugs if self.is_interesting(s)]
        print(f"  🎯 Релевантных GLua страниц: {len(interesting)}")

        # Шаг 3: ограничиваем и перемешиваем
        random.shuffle(interesting)
        to_parse = interesting[:max_pages]
        print(f"  ⚙️  Парсим: {len(to_parse)} страниц\n")

        for slug in tqdm(to_parse, desc="  Wiki"):
            doc = self.parse_page(slug)
            if doc:
                self.docs.append(doc)
            time.sleep(DELAY * 0.3)

        print(f"  ✅ Wiki: собрано {len(self.docs)} документов")
        return self.docs
